...
# BF*#URECdn-NF*RECBYUENCE-BY*#RCYEN&E*C-FN*REC*EiNEC( for the unknown author
        ...
        for author in self.data["sort_by_bot_authors"]:
            for bot in self.data["sort_by_bot_authors"][author]:
                ...
                if (search_name in bot_by_author) and (bot_by_author not in done_searching):
                    ...
                    link = self.data["sort_by_bot_authors"][author][bot]["url"]
                    if author not in list_of_possible_bots2:
                        list_of_possible_bots2[author] = []
                    list_of_possible_bots2[author].append([bot, link])
        if not list_of_possible_bots2:
            done_searching = []
...
x = DataBase.database_update("web")
...
# Tools
def embed_multi_link(title, block_title, embed_description, list_var):
    # Properties
    st = "\u200b" # Spacer title
    bot_list = ""
    inline = True
...
# def embed_multi_link2(ctx, keyword, bot_results):
#     # Properties
#     st = "\u200b" # Spacer title
#     bot_list = ""
#     inline = True
...
#     title = "Bot Results"
#     desc = "The following matches your keyword: `{}`".format(keyword)

#     embedVar = discord.Embed(title=title, description=desc, color=block_color)

#     for author in bot_results:
#         target = chunks_list(bot_results[author], 45)
...
#                     counts["field"] = 0
#                 if counts["item"] == 8:
#                     embedVar.add_field(name=author, value=bot_list, inline=inline)
#                     counts["field"] += 1
#                     counts["item"] = 0
#                     bot_list = ""
...
async def on_ready():
    ...
    description = "A discord bot that takes bots from the AutoQuestWorld's portal site and sends them to the Discord Server."
    await bloom_bot.change_presence(status=discord.Status.idle, activity=discord.Game(name="A bot Created by Bloom Autist. Currently Beta V.1.4.0.", description=description, type=3))

    # Setting `Playing ` status
    # await bloom_bot.change_presence(activity=discord.Game(name="a game"))

    # Setting `Streaming ` status
    # await bloom_bot.change_presence(activity=discord.Streaming(name="Version 1.4.0", url="Chungus.com"))

    # # Setting `Listening ` status
    # await bloom_bot.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name="a song"))

    # # Setting `Watching ` status
    # await bloom_bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="a movie"))


desc_dict = {
    "verify": "This command allows bot author to be recognized by the Sorting algorithm of Bloom Bot."\
    "Simply use `;verify author @author` to use the command. Do remember that the author must "\
    "consistently use the saved name in their .gbot files or it will fuck up."
}

# Start of Commands
@bloom_bot.command()
async def verify(ctx, *, value: str="", brief='Author Verification command', description=desc_dict["verify"]):
...
async def update(ctx, *, value: str=""):
    ...
    if priveleged:
        if not database_updating:
            ...
            await ctx.send(r"\> Updating Bloom Bot")
            if value == "web":
                result = DataBase.database_update("web")
            elif value == "html":
                result = DataBase.database_update("html")
            else:
                await ctx.send(r"\> Enter correct value")
                return
            if result:
...
async def b(ctx, *, value: str=""):
    ...
    if allowed:
        ...
        if value.lower() == "all":
            ...
            for author in bot_results:
                ...
                for bot_chunk in target:
                    ...
                    for items in bot_chunk:
                        ...
                        counts["item"]+=1                            # Bot Name, # Link, # Author
                        counts["total"] += 1
                        # pprint("{}: {}".format(author, counts["field"]))
                        # if counts["field"] == 1:
            ...
            #     desc = "The following matches your keyword: `{}`".format(bot_name)
            #     for bot_chunk in target:
            #         await ctx.send(embed=embed_multi_link("Bot Results", author, desc, bot_chunk))
            # return
    else:
...
async def a(ctx, *, value: str=""):
    ...
    if found_author:
        ...
        for bot_set in target:
            ...
            # def embed_multi_link(title, block_title, embed_description, list_var):
            await ctx.send(embed=embed_multi_link("Bot Author Result", "Link", desc, bot_set))
        return
    else:
